...
extern const NSTimeInterval kAGAnimatedImageDelayTimeIntervalMinimum;

//
//  An `AGAnimatedImage`'s job is to deliver frames in a highly performant way and works in conjunction with `AGAnimatedImageView`.
...
//
@interface AGAnimatedImage : NSObject
...
// Pass either a `UIImage` or an `AGAnimatedImage` and get back its size
+ (CGSize)sizeForImage:(id)image;

// On success, the initializers return an `AGAnimatedImage` with all fields initialized, on failure they return `nil` and an error will be logged.
...
typedef NS_ENUM(NSUInteger, AGLogLevel) {
    AGLogLevelNone = 0,
    AGLogLevelError,
    AGLogLevelWarn,
    AGLogLevelInfo,
...
@interface AGAnimatedImage (Logging)

+ (void)setLogBlock:(void (^)(NSString *logString, AGLogLevel logLevel))logBlock logLevel:(AGLogLevel)logLevel;
+ (void)logStringFromBlock:(NSString *(^)(void))stringBlock withLevel:(AGLogLevel)level;

@end

#define AGLog(logLevel, format, ...) [AGAnimatedImage logStringFromBlock:^NSString *{ return [NSString stringWithFormat:(format), ## __VA_ARGS__]; } withLevel:(logLevel)]

@interface AGWeakProxy : NSProxy
